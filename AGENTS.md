...
`ankerctl` is a CLI and Flask web UI for monitoring, controlling and printing to AnkerMake
M5 / M5C printers without Anker's closed-source software. It ships `libflagship`, the
protocol library (PPPP, MQTT, HTTP) it is built on, in the same repository.

**License:** GPL-3.0 · **Python:** >= 3.10 · **Entry point:** `ankerctl.py`

> This file is the single source of agent guidance for this repository. `CLAUDE.md` and
> `GEMINI.md` are symlinks to it. It combines the upstream `CLAUDE.md` and `GEMINI.md`
> with this fork's own findings; universal engineering rules come from the shared
> Master-Claude core imported above and are deliberately not repeated here.

### Quick reference

| | |
|---|---|
| **Install** | `pip install -r requirements.txt` (clone with `--recursive`; `transwarp/` is a submodule) |
| **Run** | `./ankerctl.py webserver run` -> http://localhost:4470 · `./ankerctl.py --help` |
| **Test** | `make check` (compileall + pytest) — the gate CI runs. `python -m pytest -q` for tests alone |
| **Regenerate** | `make diff` to preview, `make update` to write (see Build System below) |
| **Lint** | No enforced formatter or linter. Keep diffs tight and match surrounding style |

### Fork lineage

This repository is `doublegate/ankermake-m5-protocol`. On 2026-09-12 it **adopted the
`Django1982/ankermake-m5-protocol` tree as its baseline**, replacing its previous descent
from `anselor/ankermake-m5-protocol` (the "exiles" line). Both are forks of the original
`Ankermgmt/ankermake-m5-protocol` by Christian Iversen. GPL-3.0 throughout — upstream
copyright notices must be preserved. Full credits: README "Lineage and attribution".

Remotes: `origin` = doublegate · `django` = Django1982 (tracked upstream) · `upstream` =
anselor (historical). This fork's own divergence: `git log --oneline django/master..HEAD`.
...
| Path | Contents |
|---|---|
| `ankerctl.py` | CLI entry point; Click groups `mqtt`, `pppp`, `http`, `config`, `webserver` |
| `libflagship/` | Protocol layer — `pppp.py`/`ppppapi.py`, `mqtt.py`/`mqttapi.py`, `httpapi.py`, `seccode.py` + `megajank.py` (crypto), `logincache.py`, `cyclic.py` |
...
| `web/service/` | Long-lived workers — `mqtt`, `pppp`, `video`, `filetransfer`, `filament`, `history`, `homeassistant`, `timelapse` |
| `static/` | Web UI assets **and** the Jinja templates (see the `template_folder` gotcha below) |
| `templates/` | transwarp codegen templates — nothing to do with Flask |
| `specification/` | `.stf` protocol specs, the codegen input |
| `tests/` | pytest suite, 31 modules |
| `hassio-addon/` | Home Assistant add-on packaging |
| `offline/` | Offline MQTT broker (Mosquitto) for local testing |
| `examples/` | Standalone protocol scripts |
| `packaging/` | PyInstaller spec and assets |
| `documentation/` | Install guides and developer docs |

> The upstream text here pointed at `.claude/agent-memory/INDEX.md` for a fuller annotated
> index. That path is gitignored and is not present in a clone, so the table above replaces
> it. If you regenerate that index locally it will not be shared with anyone else.
...
Things that have actually cost time here. Most are not visible from the code.

- **Flask's `template_folder` is `static/`, not `templates/`.** `web/__init__.py` constructs
  `Flask(..., static_folder="static", template_folder="static")`, so the Jinja templates are
  `static/base.html`, `static/index.html`, `static/macro.html`, `static/tabs/`. The root
  `templates/` directory holds transwarp codegen templates exclusively. Check which you mean
  before editing — the names invite exactly the wrong guess.

- **Four files are generated, not hand-written:** `libflagship/pppp.py`, `libflagship/mqtt.py`,
  `libflagship/amtypes.py` and `static/libflagship.js`. Edit `specification/*.stf` or the
  templates in `templates/`, then `make update` (`make diff` previews). Editing the generated
  file directly is silently undone by the next codegen run.

- **`transwarp/` is a git submodule.** A clone without `--recursive` leaves it empty and
  `make update` fails. `make install-tools` runs `git submodule update --init` then installs it.
...
- **Printer I/O belongs in a service, never in a request handler.** `web/lib/service.py`
  provides `ServiceManager`, `RunState` and the holdoff/restart signalling; the workers in
  `web/service/` are what actually touch printers. Websocket routes only pump their queues.
...
- **`tests/test_print_history.py` is flaky on Python 3.10/3.11.** Intermittent
  ...
  `web/service/history.py` racing the test's own connection. It surfaces as a *failure* on the
  3.10 CI job and as an unhandled-thread-exception *warning* on 3.13/3.14. A re-run of the
  identical commit passes, and upstream's CI is green on the same commit — so treat a lone red
  3.10 job as the race, re-run it, and investigate only if it reproduces. The underlying bug is
  real and unfixed.

- **`login.json` is credential material.** Its `user_id` authenticates to the MQTT broker and
  works as a password, which is why `config show` redacts it. It is gitignored here. Never
  commit one, and never paste `config show` / `config decode` output into an issue.

- **Upstream issue and PR citations must keep pointing upstream.** The links in `CHANGELOG.md`
  and `documentation/issue77_code_fix.md` (#77, #85, #89) refer to issues in Django1982's
  repository and exist nowhere else; retargeting them would produce dead links and misattribute
  the work. `.github/FUNDING.yml` likewise still names the upstream maintainer, deliberately —
  a comment in the file records why.

- **A forked repository suppresses GitHub Actions behind a one-time acknowledgement banner**
  in the Actions tab, and no API clears it. Every programmatic signal lies while the gate is
  shut: `actions/permissions` reports `enabled: true`, `gh workflow list` shows the workflow
  `active`, and `gh workflow enable` exits zero. The only reliable indicator is `total_count: 0`
